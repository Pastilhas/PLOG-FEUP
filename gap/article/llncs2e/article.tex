...
\paragraph{}
Initially, we opted to use an ordering function to order the coordinates. This way, we would only get one solution and not
permutations of the same solution. Later, while doing timed tests, we noticed irregularities. To combat these, we found
a new way to search solutions, a random function. After adding this function to our project and removing the precious
ordering one, we noticed great improvement in our tests.

\begin{figure}
    \begin{center}
        \includegraphics[scale=0.4]{images/random.png}
...
\begin{figure}
    \begin{center}
        \includegraphics[scale=0.6]{images/graph2.png}
